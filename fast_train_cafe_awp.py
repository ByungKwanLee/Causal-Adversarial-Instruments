...
def test(net, testloader, attack, rank):
    ...
    if acc > best_acc:
        ...
        best_acc = acc
        if rank == 0:
            torch.save(state, './checkpoint/pretrain/%s/%s_cafeawp_%s%s_best.t7' % (args.dataset, args.dataset,
                                                                                args.network,
                                                                                args.depth))
            print('Saving~ ./checkpoint/pretrain/%s/%s_cafeawp_%s%s_best.t7' % (args.dataset, args.dataset,
                                                                            args.network,
                                                                            args.depth))
...
def main_worker(rank, ngpus_per_node=ngpus_per_node):
    ...
    # Load AWP Network
    checkpoint_name = 'checkpoint/pretrain/%s/%s_awp_%s%s_best.t7' % (args.dataset, args.dataset, args.network, args.depth)
