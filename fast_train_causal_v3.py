...
def causal_train(epoch, net, c_net, z_net, trainloader, c_optimizer, inst_optimizer, c_scheduler, z_scheduler, scaler, attack, gpu, writer):
    global counter
    print('\nEpoch: %d' % epoch)
...
def main_worker(gpu, ngpus_per_node=ngpus_per_node):
    ...
    checkpoint = torch.load('checkpoint/pretrain/%s/%s_adv_%s%s_best.t7' % (args.dataset, args.dataset, args.network, args.depth))
    ...
    for epoch in range(args.epoch):
        causal_train(epoch, net, c_net, z_net, trainloader, c_optimizer, inst_optimizer, c_scheduler, z_scheduler, scaler, attack, gpu, writer)
