...
svhn_adv = np.array([[0.6497, 0.5266, 0.4839, 0.704, 0.5547, 0.5177]])
svhn_inst = np.array([[0.4909, 0.4566, 0.3811, 0.4369, 0.403, 0.436]])
svhn_causal = np.array([[0.8784, 0.8848, 0.8994, 0.9186, 0.9225, 0.9122]])
svhn_treat = np.array([[0.913, 0.9141, 0.9101, 0.9304, 0.9344, 0.9242]])

svhn = np.concatenate([svhn_adv, svhn_inst, svhn_causal, svhn_treat])

svhn_df = pd.DataFrame(columns=content, data=svhn)

svhn_ = svhn_df.set_index([["Adv", "Inst", "Causal", "Treat"]])
