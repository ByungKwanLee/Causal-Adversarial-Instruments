...
# model parameter
parser.add_argument('--NAME', default='CAFE-HELP', type=str)
...
def train(net, std, c_net, trainloader, optimizer, lr_scheduler, scaler, inv_causal, attack, awp):
    net.train()
    std.eval()
    c_net.eval()
    train_loss = 0
    ...
    for batch_idx, (inputs, targets) in prog_bar:
        ...
        with autocast():
            ...
            loss = trades_loss(clean_outputs, adv_outputs, targets)\
                   +0.25*F.cross_entropy(help_outputs, help_target) + causal_loss(adv_outputs, inv_outputs)
...
def test(net, testloader, attack, rank):
    ...
    if acc > best_acc:
        ...
        best_acc = acc
        if rank == 0:
            torch.save(state, './checkpoint/causal/%s/%s_cafehelp_%s%s_best.t7' % (args.dataset, args.dataset,
                                                                                args.network,
                                                                                args.depth))
            print('Saving~ ./checkpoint/causal/%s/%s_cafehelp_%s%s_best.t7' % (args.dataset, args.dataset,
                                                                            args.network,
                                                                            args.depth))
...
def main_worker(rank, ngpus_per_node=ngpus_per_node):
    ...
    for epoch in range(args.epoch):
        ...
        train(net, std, c_net, trainloader, optimizer, lr_scheduler, scaler, inv_causal, attack, awp)
