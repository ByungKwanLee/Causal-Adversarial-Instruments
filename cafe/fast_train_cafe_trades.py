...
# model parameter
parser.add_argument('--NAME', default='CAFE-TRADES', type=str)
...
def test(net, testloader, attack, rank):
    ...
    if acc > best_acc:
        ...
        best_acc = acc
        if rank == 0:
            torch.save(state, './checkpoint/causal/%s/%s_cafetrades_%s%s_best.t7' % (args.dataset, args.dataset,
                                                                                args.network,
                                                                                args.depth))
            print('Saving~ ./checkpoint/causal/%s/%s_cafetrades_%s%s_best.t7' % (args.dataset, args.dataset,
                                                                            args.network,
                                                                            args.depth))
...
def main_worker(rank, ngpus_per_node=ngpus_per_node):
    ...
    for epoch in range(args.epoch):
        ...
        train(net, c_net, trainloader, optimizer, lr_scheduler, scaler, inv_causal, attack)
