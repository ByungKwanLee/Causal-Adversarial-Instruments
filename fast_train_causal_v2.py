...
def causal_train(epoch, net, c_net, z_net, trainloader, c_optimizer, inst_optimizer, c_scheduler, z_scheduler, scaler, attack, gpu,
                 writer):
    global counter
    print('\nEpoch: %d' % epoch)
...
def causal_train(epoch, net, c_net, z_net, m_net, trainloader, c_optimizer, inst_optimizer, c_scheduler,
                     z_scheduler, scaler, attack, gpu, writer):
    global counter
    print('\nEpoch: %d' % epoch)
...
def main_worker(gpu, ngpus_per_node=ngpus_per_node):
    ...
    for epoch in range(args.epoch):
        causal_train(epoch, net, c_net, z_net, trainloader, c_optimizer, inst_optimizer, c_scheduler,
                     z_scheduler, scaler, attack, gpu, writer)
        causal_test(epoch, net, c_net, z_net, testloader, criterion, attack, gpu)

        c_scheduler.step(), z_scheduler.step()
