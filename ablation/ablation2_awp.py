...
# model parameter
parser.add_argument('--NAME', default='Ablation2-AWP', type=str)
...
def test(net, testloader, attack, rank):
    ...
    if acc > best_acc:
        ...
        best_acc = acc
        if rank == 0:
            torch.save(state, './checkpoint/ablation/%s/%s_ablation2awp_%s%s_best.t7' % (args.dataset, args.dataset,
                                                                                args.network,
                                                                                args.depth))
            print('Saving~ ./checkpoint/ablation/%s/%s_ablation2awp_%s%s_best.t7' % (args.dataset, args.dataset,
                                                                            args.network,
                                                                            args.depth))
