...
# model parameter
parser.add_argument('--NAME', default='Ablation2-MART', type=str)
...
def train(net, c_net, trainloader, optimizer, lr_scheduler, scaler, attack):
    ...
    for batch_idx, (inputs, targets) in prog_bar:
        ...
        with autocast():
            ...
            loss = mart_loss(clean_outputs, adv_outputs, targets) + (adv_feature-causal_feature.detach()).square().mean()
...
def test(net, testloader, attack, rank):
    ...
    if acc > best_acc:
        ...
        best_acc = acc
        if rank == 0:
            torch.save(state, './checkpoint/ablation/%s/%s_ablation2mart_%s%s_best.t7' % (args.dataset, args.dataset,
                                                                                args.network,
                                                                                args.depth))
            print('Saving~ ./checkpoint/ablation/%s/%s_ablation2mart_%s%s_best.t7' % (args.dataset, args.dataset,
                                                                            args.network,
                                                                            args.depth))
