...
cifar_adv = np.array([[0.4984, 0.4474, 0.4322, 0.5215, 0.475, 0.4554]])
cifar_inst = np.array([[0.2418, 0.2447, 0.2447, 0.1574, 0.1609, 0.1755]])
cifar_causal = np.array([[0.7789, 0.7738, 0.7188, 0.807, 0.7976, 0.7975]])
cifar_treat = np.array([[0.7939, 0.7965, 0.7845, 0.838, 0.8431, 0.8148]])

cifar = np.concatenate([cifar_adv, cifar_inst, cifar_causal, cifar_treat])

cifar_df = pd.DataFrame(columns=content, data=cifar)

cifar_ = cifar_df.set_index([["Adv", "Inst", "Causal", "Treat"]])
